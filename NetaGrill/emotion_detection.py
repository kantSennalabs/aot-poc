...
def web_cam(face_detector,model,src,vid_rec = False):
    """
    ...
    for index in range(6):
        ...
        emoji.append(cv2.imread('../facial_recognition/emojis/' + emotion + '.png', -1))
            ...
            try:
                ...
                if len(faces) > 0:
                    for x, y, width, height in faces:
                        ...
                        path = "cap_img/"
                        img_path = os.path.join(path, f"{predicted_emotion}{datetime.datetime.now().strftime('%d-%m-%Y-%H-%M-%S')}.jpg")
                        cv2.imwrite(img_path , cv2.resize(frame, (0,0), fx= 0.2, fy=0.2))
                        job = q.enqueue(loging, img_path, predicted_emotion) 
...
def main():
    ...
    face_detector = cv2.CascadeClassifier('../facial_recognition/model/haarcascade_frontalface_default.xml')
    emotion_model = load_model('../facial_recognition/model/emotion_recognition.h5')
    # web_cam(face_detector,emotion_model,"rtsp://admin:Sennalabs_@192.168.0.63/Streaming/Channels/101")
    web_cam(face_detector,emotion_model,0)
